...
import { collection, deleteDoc, deleteField, doc, getDocs, getFirestore, updateDoc } from 'firebase/firestore';
...
const cityDistrictMap = {
    'İstanbul': ['Kadıköy', 'Beşiktaş', 'Üsküdar', 'Şişli', 'Maltepe', 'Fatih', 'Beyoğlu', 'Sarıyer', 'Kartal'],
    ...
    'Kocaeli': ['İzmit', 'Gebze', 'Darıca', 'Gölcük'],
...
async function standardizeUsers() {
    ...
    for (const userDoc of userSnapshot.docs) {
        ...
        if (!data.address || data.address === 'Bilinmiyor') {
            const cities = Object.keys(cityDistrictMap);
            const randomCity = cities[Math.floor(Math.random() * cities.length)];
            const districts = cityDistrictMap[randomCity];
            const randomDistrict = districts[Math.floor(Math.random() * districts.length)];
            updates.address = `${randomCity}, ${randomDistrict}`;
        }
