            ...
            <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 0 }}>
                <View style={styles.profileTop}>
                    ...
                    <View style={styles.badgeRow}>
                        ...
                        {(() => {
                            // Satıcının profil adresi veya ilanlarından konum bilgisi
                            let loc = null;

                            // 1. Satıcının city ve district bilgisine bak
                            if (seller?.city && seller?.district) {
                                loc = `${seller.city} / ${seller.district}`;
                            }
                            // 2. Satıcının sadece city bilgisine bak
                            else if (seller?.city) {
                                loc = seller.city;
                            }
                            // 3. Satıcının address bilgisine bak (eğer city/district yoksa)
                            else if (seller?.address && seller.address !== 'Bilinmiyor' && seller.address !== 'Unknown') {
                                loc = seller.address;
                            }

                            // 4. Eğer profil adresi yoksa, ilk ilanın konumunu kullan
                            if (!loc) {
                                if (products && products.length > 0) {
                                    const p = products[0];
                                    if (p.city && p.district) {
                                        loc = `${p.city} / ${p.district}`;
                                    } else if (p.city) {
                                        loc = p.city;
                                    } else if (p.location) {
                                        loc = p.location;
                            ...
                            // Geçerli bir konum varsa göster
                            if (loc && loc !== 'Bilinmiyor' && loc !== 'Unknown') {
                                return (
                ...
                {products.length === 0 ? (
                    ...
                    <View style={styles.productsGrid}>
                        {products.map((p) => (
                            <View key={p.id} style={{ width: `${100 / numColumns}%`, padding: 4 }}>
                                <ProductCard
                                    title={p.title}
